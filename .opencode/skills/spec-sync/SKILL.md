...
You are a **sync verification agent** for the Servoy QR Code project. Your job is to
...
- A specific component name (e.g., `scanner`, `generator`)
...
If a specific component is given, use that. If `all`, check all components.

Component mapping:

| Component | Spec file | Angular implementation | AngularJS |
|-----------|-----------|----------------------|-----------|
| scanner | `svyqrcode/svyqrcode.spec` | `svyqrcode/projects/svyqrcode/src/scanner/scanner.ts` | `svyqrcode/svyqrcode/svyqrcode.js` |
| generator | `svyqrcodegenerator/svyqrcodegenerator.spec` | `svyqrcode/projects/svyqrcode/src/generator/generator.ts` | — (Angular-only; `.js` is a stub) |

**Important:**
- `svyqrcode.spec` defines the contract for BOTH the Angular `SvyQRCodeScanner` and the
  AngularJS scanner directive.
- `svyqrcodegenerator.spec` is Angular-only; its `.js`/`.html` are minimal stubs.

### Step 2 — For each component, read both layers

1. **Spec file:** Parse the JSON from the component's `.spec`
2. **Angular file:** Read the `.ts` component file
3. **AngularJS file (scanner only):** Read the `.js` file
...
| **Missing in AngularJS** | Model property in spec but missing in JS controller (scanner only) | Property name |
...
- `designsize` with `"tags": { "serveronly": true }` — designer-only default dimensions, NOT an input
...
- Handlers in the `.spec` map to function-callback `input()`s in the Angular component
  (e.g., `onCodeDetected`, `onError`) — a missing callback input for a spec handler is a mismatch.
- Properties with `pushToServer: "allow"` or `"deep"` that use a `model()` signal for
  two-way binding are a valid pattern.
