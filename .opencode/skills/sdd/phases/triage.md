...
Key areas to investigate for this QR Code component library:
- The Servoy component spec file (`svyqrcode/svyqrcode.spec` or
  `svyqrcodegenerator/svyqrcodegenerator.spec`) — defines the component contract
  (model properties, handlers, API methods, types)
- The Angular implementation (`svyqrcode/projects/svyqrcode/src/scanner/` or
  `svyqrcode/projects/svyqrcode/src/generator/`)
- The AngularJS scanner implementation (`svyqrcode/svyqrcode/svyqrcode.js`)
- The `@servoy/public` base classes and utilities (e.g. `ServoyBaseComponent`)
...
```bash
cd /home/gabi/github/svyQRCode ; git blame -L <start>,<end> "svyqrcode/<file-path>"
