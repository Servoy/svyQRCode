...
function validatePackageSync(workdir: string): string[] {
  ...
  const rootPkgPath = resolve(workdir, "svyqrcode/package.json")
  const distPkgPath = resolve(workdir, "svyqrcode/projects/svyqrcode/package.json")
          ...
          `Commit validation failed:\n${allErrors.map((e) => `  - ${e}`).join("\n")}\n\n` +
            (requireJiraKey
              ? `Expected format: <JIRA_KEY> <short description> [ai]\n` +
                `Example: SVY-22100 add QR code generator component [ai]`
              : `Expected format: <short description> [ai]\n` +
                `Example: add QR code generator component [ai]`)
        )
      }
    },
  }
